...
type FormFields = {
    ...
    willEngage: boolean | null
    reachCategory: ReachCategory | ""
    wantsPhoto: boolean | null   // null = not yet answered
    photoDataUrl: string | null
}
...
export default function PledgeForm({ onSubmit, isSubmitting }: Props) {
    ...
    const handleWantsPhoto = (wants: boolean) => {
        set("wantsPhoto", wants)
        // If they switch to No, clear any uploaded photo
        if (!wants) {
            set("photoDataUrl", null)
            if (fileInputRef.current) fileInputRef.current.value = ""
        }
    ...
    const validate = (): boolean => {
        ...
        if (!fields.reachCategory)
            e.reachCategory = "Please select your reach"
        if (fields.wantsPhoto === null)
            e.wantsPhoto = "Please answer this question"
        // Photo only required if they said Yes
        if (fields.wantsPhoto === true && !fields.photoDataUrl)
            e.photoDataUrl = "Please upload your photo or select No above"
        setErrors(e)
        return Object.keys(e).length === 0
    }
    ...
    const handleSubmit = () => {
        ...
        onSubmit({
            ...
            reachCategory,
            starCount: REACH_TO_STARS[reachCategory],
            photoDataUrl: fields.wantsPhoto ? fields.photoDataUrl : null,
        })
    }
    ...
    return (
        <div className="w-full max-w-2xl mx-auto">
            ...
            <div className="relative bg-white rounded-3xl shadow-xl shadow-gray-100 border border-gray-100 p-8 space-y-6">
                ...
                {/* Photo preference question */}
                <Field
                    label="Would you like your photo on your badge?"
                    error={errors.wantsPhoto}
                    required
                >
                    <div className="flex gap-3 mt-1">
                        {(["Yes", "No"] as const).map((option) => {
                            const val = option === "Yes"
                            const active = fields.wantsPhoto === val
                            return (
                                <button
                                    key={option}
                                    type="button"
                                    disabled={isSubmitting}
                                    onClick={() => handleWantsPhoto(val)}
                                    className={`flex-1 py-3 rounded-xl text-sm font-bold border-2 transition-all duration-200 ${active
                                        ? "border-green-500 bg-green-50 text-green-800"
                                        : "border-gray-200 bg-white text-gray-500 hover:border-green-300"
                                        }`}
                ...
                {/* Photo upload — only shown when wantsPhoto === true */}
                {fields.wantsPhoto === true && (
                    <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">
                            Upload Your Photo
                        ...
                        {errors.photoDataUrl && <ErrorMsg message={errors.photoDataUrl} />}
                    </div>
                )}

                {/* No-photo notice */}
                {fields.wantsPhoto === false && (
                    <div className="flex items-start gap-3 bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3">
                        ...
                        <p className="text-xs text-gray-500 leading-relaxed">
                            No problem — your badge will use your initials instead of a photo.
                            You can still share it proudly!
                        </p>
                    </div>
                )}
